...
- 폐기 시점: 분석 성공 · 안전차단(SafetyBlocked) · 사용자의 명시적 삭제 · TTL 만료
  - 분석 터미널·배너 [삭제] 시 `__draft__` 이미지 파일도 디렉토리째 삭제한다
  - 이미지 개별 제거 시에는 해당 승격본만 삭제한다 — 목록에 남은 참조가 없을 때에 한하며, 승격 실패로 폴백한 picker 원본 경로는 사용자 파일이라 지우지 않는다
  - ⚠️ 남은 고아 경로 2가지: ① TTL 만료 폐기는 초안 슬롯만 지운다 (`GetDiaryDraftUseCase`는 순수 Dart라 `ImageService`를 호출할 수 없다) ② 승격 복사 직후 화면이 unmount되면 그 파일이 목록에 들어가지 못한 채 남는다. 둘 다 다음 터미널 분석·배너 [삭제] 시 디렉토리째 정리된다
