...
### Step 6.1: tasks.md 크기 점검 (자동 아카이브)

1. 라인 수 확인: `wc -l docs/tasks.md`
2. 완료 태스크 수: `grep -c '^\- \[x\]' docs/tasks.md`

트리거 기준:
- tasks.md **≥ 150줄** 또는 **[x] 태스크 ≥ 5개** → 아카이브 실행

아카이브 절차:
```
a. 현재 버전 + 날짜로 섹션 헤더 생성: ## v{VERSION} ({DATE})
b. [x] 태스크 블록을 docs/tasks/history.md 상단(최신 먼저)에 prepend
c. tasks.md에서 해당 블록 삭제
d. tasks.md 헤더 최종 업데이트 날짜 갱신
```

**history.md 크기 점검** (P1):
```bash
wc -l docs/tasks/history.md
```
- **≥ 300줄** → 연월별 분할 제안:
  ```
  ⚠️ history.md가 N줄로 커졌습니다.
  → docs/tasks/history-2026-01.md 등 월별 분할 고려하세요.
  ```
...
🔵 변경 없음: [x] 태스크 없음 (skip)
⚠️ 임박: tasks.md {N}줄 (상한 150줄까지 {X}줄 여유)
📦 history.md: {N}줄 (300줄 도달 시 월별 분할 권장)
```
