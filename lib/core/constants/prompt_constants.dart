...
❌ "심호흡하고 하기" → ✅ "심호흡하기" (중복 동사 제거)
❌ "산책하고 하기" → ✅ "산책하기" (중복 동사 제거)
...
2. 'empathy_message'는 존댓말을 사용하며, 선택된 캐릭터의 어조에 맞게 작성하세요.
   ...
   - 반드시 한국어로만 작성하세요.
   - 실제 사전에 있는 단어만 사용하세요. 없는 단어 조합(예: "마음을춘") 절대 금지.
   - 어색하면 "마음을 다잡으신", "마음을 추스르신" 같은 표준 표현을 사용하세요.
...
3. 'action_items'는 단계별 추천 행동 3개를 배열로 제공합니다:
   ...
   - 반드시 '~하기', '~기' 형태의 명사형으로 끝내세요.
   - 금지: '~하고 하기', '~해서 하기' 같은 중복 동사 패턴.
...
- 'action_items'의 첫 번째 항목에 "전문 상담사와 대화해 보세요. 109로 연락하세요."
...
''';
  ...
  static String createAnalysisPromptWithImages(
    String diaryContent, {
    required int attachedImageCount,
    required int analyzedImageCount,
    AiCharacter character = AiCharacter.warmCounselor,
    String? userName,
  }) {
    final timeSlot = _getTimeSlotName();
...
'''
    ...
    final visionScopeNote = attachedImageCount > analyzedImageCount
        ? 'AI 이미지 분석에는 대표 사진 $analyzedImageCount장만 반영됩니다. '
        : '';
...
[첨부 이미지]
사용자가 $attachedImageCount장의 사진을 첨부했습니다. $visionScopeNote첨부된 이미지와 텍스트를 종합하여 감정을 분석해주세요.
