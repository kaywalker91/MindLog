...
## 2026-08-25 - 파이프 뒤 `$?`가 샌드박스 실패를 "품질 게이트 통과"로 위장했다
**무엇이 잘못됐나**: v1.4.63 작업 중 `fvm flutter analyze 2>&1 | tail -15; echo "analyze rc=$?"` 로 `rc=0` 을 얻어 "analyze 통과"로 보고했다. 실제로는 analyze 가 **실행조차 되지 않았다**. `dart format` 도 같은 방식으로 "42개 파일 변경"이라는 잘못된 결과를 냈다.
**근본 원인**: 두 겹의 오류가 겹쳤다. (1) 파이프라인 뒤의 `$?` 는 **마지막 명령(`tail`)의 종료코드**다 — 앞 명령이 실패해도 0이 나온다. (2) 샌드박스가 `/Users/kaywalker/flutter/bin/cache/engine.stamp` 쓰기를 막아 `update_engine_version.sh` 가 `Operation not permitted` 로 죽었고, 그 뒤 flutter 명령이 실행되지 않았다. 출력이 `tail` 로 잘려 에러 한 줄만 남으니 "이슈 없음"처럼 보였다.
**해결책**: 샌드박스를 해제하고 `cmd > file 2>&1; echo "rc=$?"` 형태로 **파이프 없이** 종료코드를 잡아 재실행 → analyze `rc=0` (`No issues found!`), format 은 실제로 43개 파일 드리프트. 임시 파일도 `/tmp` 가 아닌 스크래치패드로 옮겨야 했다(`/tmp` 쓰기도 차단됨).
**예방 규칙**:
1. **품질 게이트 종료코드는 파이프 뒤에서 읽지 마라.** `cmd > log 2>&1; rc=$?` 로 잡고 로그는 따로 본다. zsh 에서 `${PIPESTATUS[0]}` 도 안 통한다(zsh 는 `$pipestatus`).
2. flutter/dart 명령에서 `engine.stamp: Operation not permitted` 가 보이면 **그 명령은 실행되지 않은 것**이다. rc 를 믿지 말고 샌드박스 해제 후 재실행한다.
3. "rc=0" 을 근거로 통과를 보고하기 전에 **성공 시 나와야 할 출력**(`No issues found!`, `All tests passed!`)이 실제로 있는지 확인한다. 종료코드와 기대 출력을 함께 봐야 한다.

## 2026-08-25 - cd.yml `paths-ignore` 가 양방향으로 틀렸다 (필요한 빌드 누락 + 불필요한 빌드 유발)
**무엇이 잘못됐나**: 같은 날 같은 설정이 정반대 방향으로 두 번 문제를 냈다. (1) `docs/legal/privacy-policy.md` 는 **앱 번들 asset**인데 `paths-ignore` 의 `**.md`·`docs/**` 에 걸려 CD 가 안 돌았다 → versionCode 70 AAB 에 틀린 고지 문구가 실렸다. (2) 반대로 `android/fastlane/metadata/**/*.txt`(출시 노트)는 **앱에 아무 영향이 없는데** paths-ignore 대상이 아니라 CD 를 트리거한다 → 재빌드된 versionCode 71 이 이미 Internal 에 존재해 중복으로 거부된다.
**근본 원인**: `paths-ignore` 가 "빌드 산출물에 영향을 주는가"가 아니라 **"문서처럼 생겼는가"** 를 기준으로 작성돼 있었다. 파일의 위치·확장자와 빌드 영향이 일치한다는 전제가 깨진 지점이 두 곳 있었다.
**해결책**: (1) 은 v1.4.63 에서 `pubspec.yaml` 변경이 CD 를 트리거해 재빌드로 해소. (2) 는 71.txt 를 커밋만 하고 **푸시 보류**, 다음 버전 범프와 함께 내보내기로 했다. 근본 수정은 미착수.
**예방 규칙**:
1. `paths-ignore` 항목을 추가·수정할 때 **`pubspec.yaml` 의 assets 목록과 대조하라.** 경로가 문서처럼 보여도 asset 이면 빌드에 영향한다.
2. GitHub Actions 는 `paths-ignore` 에서 `!` 부정 패턴을 **지원하지 않는다**(부정은 `paths` 에서만, 둘은 같은 이벤트에 병용 불가). "docs/** 는 무시하되 이 파일만 예외"는 표현 불가 → `paths` 화이트리스트 전환 / asset 을 `docs/` 밖으로 이동 / 방침 수정 시 버전 범프 강제 체크 중 택1.
3. 이미 업로드된 versionCode 를 재빌드·재업로드하면 Play 가 거부한다. **승격은 새 빌드가 아니라 기존 아티팩트 이동**이어야 한다 — `fastlane deploy_production` 은 `build` 를 선행 호출하므로 승격 용도로 쓰면 안 된다.

## 2026-08-25 - 백로그가 확인하지 않은 상태를 단언해 불필요한 작업을 만들었다
**무엇이 잘못됐나**: `.claude/progress/current.md` 가 `Health apps declaration` 을 "미완료"로 적어 두어, 다음 세션이 이를 High 인접 작업으로 잡고 Play Console 을 열어 확인했다. **실제로는 이미 제출돼 있었다.**
**근본 원인**: 이전 세션이 콘솔을 확인하지 않고 추정으로 상태를 적었다. 백로그는 다음 세션에 **검증된 사실로 읽힌다** — 추정과 사실이 같은 문법으로 적히면 구분할 방법이 없다.
**해결책**: 항목을 삭제하고 주의사항에 경위를 기록. 부수적으로 「정신 및 행동 건강」 항목을 추가해 선언 정확도는 올렸다(앱이 CBT 기법·인지왜곡 분류·위기 감지를 실제 구현하므로 해당).
**예방 규칙**: 외부 콘솔(Play Console, Firebase, GitHub Settings)의 상태를 백로그에 적을 때는 **확인 시점과 근거를 함께 적거나, 확인하지 않았음을 명시**하라. "미완료"와 "미확인"은 다른 말이다. v1.4.61 의 `{name}` 교훈("고친 곳과 터지는 곳이 다르다")과 같은 계열 — 검증 범위를 특정하지 않은 서술이 다음 사람을 오도한다.

## 2026-08-28 - 파일을 커밋한 것과 파이프라인이 그 파일을 쓰는 것은 다르다 (fastlane 출시 노트)
**무엇이 잘못됐나**: v1.4.64 릴리스에서 `android/fastlane/metadata/android/{ko,en-US}/changelogs/72.txt` 를 새로 작성하고, 사용자에게 **"출시 노트는 새로 넣은 72.txt 가 사용됩니다"** 라고 두 번 단언했다. 실제로는 Play 에 **업로드되지 않았다**. CD 로그의 `skip_upload_changelogs | true` 로 사후 발각.
**근본 원인**: `Fastfile` 을 열어보지 않고 **파일의 존재와 fastlane 의 관례적 동작만으로 결과를 추정**했다. `android/fastlane/Fastfile` 의 4개 레인(`deploy_internal`·`deploy_beta`·`deploy_production` 외 1) 전부가 `skip_upload_metadata: true` + `skip_upload_changelogs: true` 로 설정돼 있어, AAB 만 올리고 메타데이터·출시 노트는 일절 건드리지 않는다. 같은 추정으로 그 직전에 "`72.txt` 가 없으면 `default.txt` 의 「성능 개선 및 버그 수정」이 나간다"고도 말했는데 이 역시 성립하지 않는다 — **어느 쪽 파일도 업로드되지 않는다.**
**부수 피해 없음**: 사용자용 변경 안내의 실제 경로는 앱이 원격 조회하는 `docs/update.json`(`app_constants.dart:10`) 이고 이쪽은 GitHub Pages 배포로 정상 반영됐다. `72.txt` 는 커밋된 채로 남아 플래그를 끄는 시점에 바로 쓰인다.
**해결책**: 미착수. `skip_upload_changelogs` 를 끄는 것은 배포 파이프라인 동작 변경이라 사용자 판단 대기. 그때까지 Play Console 출시 노트는 수기 입력이어야 한다.
**예방 규칙**:
1. **"이 파일이 어디에 쓰이는가"를 소비하는 쪽 설정에서 확인하라.** `metadata/` 디렉토리에 파일이 있다는 것은 fastlane 이 그것을 업로드한다는 뜻이 아니다. `Fastfile` 의 `skip_upload_*` 플래그가 실제 동작을 결정한다.
2. 배포 결과를 사용자에게 단언하기 전에 **파이프라인 로그에서 해당 스텝의 실제 파라미터를 확인**하라. `upload_to_play_store` 는 실행 시 파라미터 표를 로그에 출력한다 — 사후가 아니라 사전에 볼 수 있었다.
3. 2026-08-25 「백로그가 확인하지 않은 상태를 단언했다」와 같은 계열. **추정과 확인된 사실을 같은 문법으로 말하지 마라.** 이번엔 백로그가 아니라 사용자에게 직접 단언했다는 점에서 더 나쁘다.

## 2026-09-01 - GitHub Pages 는 `/docs` 를 루트로 서빙한다 — 저장소 루트 asset 참조는 전부 404
**무엇이 잘못됐나**: `docs/index.html` 의 스크린샷 갤러리 8장이 **게시된 사이트에서 전부 404** 였다. 로컬에서 파일을 열면 정상이라 몇 달간 발각되지 않았다. 링크드인에서 트래픽을 보내려고 사이트를 점검하다 발견했다.
**근본 원인**: Pages 소스가 `main` 브랜치의 `/docs` 이므로 사이트 루트는 `docs/` 다. 따라서 `<img src="assets/screenshots/v3/x.jpeg">` 는 `/MindLog/assets/...` → 실제 파일은 **저장소 루트** `assets/` 에만 있어 서빙 대상 밖. README 는 저장소에서 렌더되므로 같은 상대경로가 정상 동작한다 — **같은 경로 문자열이 한쪽에서만 깨지는 것**이 발각을 늦췄다.
**해결책**: 이미지를 `docs/assets/` 로 복사하되 웹 표시 크기에 맞춰 축소(긴 변 720px, JPEG q72 → 3.4MB에서 355KB). 원본은 루트에 존치. `docs/assets/og-image.png`(1200×630) 신규 제작 후 OG/Twitter 메타 태그 추가 — 두 페이지 모두 `og:` 태그가 **0건**이라 링크 공유 시 카드가 비어 있었다.
**예방 규칙**:
1. **Pages 사이트의 링크·이미지는 라이브 URL 로 검증하라.** 로컬 파일 열기나 저장소 뷰는 서빙 경로를 재현하지 않는다. `curl -o /dev/null -w '%{http_code}'` 한 줄이면 된다.
2. 공개 문서(README·랜딩)는 **코드처럼 드리프트한다.** 이번에 함께 발견된 것: Play 배지가 존재하지 않는 패키지명(`com.mindlog.app`, 404)을 가리킴 · Recent Updates 가 22 릴리스 뒤처짐(v1.4.42, 테스트 1,505건) · AI 모델 표기가 코드와 불일치(`llama-3.3-70b` vs 실제 `openai/gpt-oss-120b`).
3. **v1.4.62 에서 온보딩의 "외부로 전송되지 않아요"를 고쳤으나 README·랜딩의 같은 문구는 남아 있었다.** 허위 고지를 고칠 때는 **그 문구가 있는 모든 표면**을 grep 하라 — 앱 화면만 고치면 공개 문서가 계속 거짓을 말한다. v1.4.61 `{name}` 교훈("고친 곳과 터지는 곳이 다르다")의 문서판.
